...
categories: [ai, conference, summary, llms]
jupyter: python3
format:
  html:
    code-overflow: wrap
...
I didn't get the chance to attend NeurIPS last week. The sheer number of papers submitted is [staggering](https://blog.neurips.cc/tag/neurips2023/): 13,300 submitted papers that were reviewed by 1,000+ people. 3,540 papers were accepted and 502 papers were flagged for ethics reviews!

The folks at the Latent Space podcast did a good job summarizing the wealth of content, one reviewing the [best papers](https://podcasts.google.com/feed/aHR0cHM6Ly93d3cubGF0ZW50LnNwYWNlL2ZlZWQ/episode/aHR0cHM6Ly93d3cubGF0ZW50LnNwYWNlL3AvbmV1cmlwcy0yMDIzLXBhcGVycw?sa=X&ved=0CAUQkfYCahcKEwiYzPGz7LiDAxUAAAAAHQAAAAAQOw) and another that was focused on [top startups](https://podcasts.google.com/feed/aHR0cHM6Ly93d3cubGF0ZW50LnNwYWNlL2ZlZWQ/episode/aHR0cHM6Ly93d3cubGF0ZW50LnNwYWNlL3AvbmV1cmlwcy0yMDIzLXN0YXJ0dXBz?sa=X&ved=0CAUQkfYCahcKEwj4-6rNmcODAxUAAAAAHQAAAAAQCg).

I transcribed these podcasts using Whisper and tried taking a stab at summarizing them. Those transcripts are here:

- [Latent Space Top Papers Transcript](./neurips_papers_2023.wav.txt)
- [Latent Space Top Startups Transcript](./neurips_startups_2023.wav.txt)

The podcast comes with timestamps for each section which is very helpful:

```python
data = [
    {"time": "0:01:19", "title": "Word2Vec", "presenter": ["Jeff Dean", "Greg Corrado"]},
    {"time": "0:15:28", "title": "Emergence Mirage", "presenter": ["Rylan Schaeffer"]},
    {"time": "0:28:48", "title": "DPO", "presenter": ["Rafael Rafailov"]},
    {"time": "0:41:36", "title": "DPO Poster Session", "presenter": ["Archit Sharma"]},
    {"time": "0:52:03", "title": "Datablations", "presenter": ["Niklas Muennighoff"]},
    {"time": "1:00:50", "title": "QLoRA", "presenter": ["Tim Dettmers"]},
    {"time": "1:12:23", "title": "DataComp", "presenter": ["Samir Gadre"]},
    {"time": "1:25:38", "title": "DataComp Poster Session", "presenter": ["Samir Gadre", "Alex Dimakis"]},
    {"time": "1:35:25", "title": "LLaVA", "presenter": ["Haotian Liu"]},
    {"time": "1:47:21", "title": "LLaVA Poster Session", "presenter": ["Haotian Liu"]},
    {"time": "1:59:19", "title": "Tree of Thought", "presenter": ["Shunyu Yao"]},
    {"time": "2:11:27", "title": "Tree of Thought Poster Session", "presenter": ["Shunyu Yao"]},
    {"time": "2:20:09", "title": "Toolformer", "presenter": ["Jane Dwivedi-Yu"]},
    {"time": "2:32:26", "title": "Voyager", "presenter": ["Guanzhi Wang"]},
    {"time": "2:45:14", "title": "CogEval", "presenter": ["Ida Momennejad"]},
    {"time": "2:59:41", "title": "State Space Models", "presenter": ["Chris Ré"]}
]
```
...
```python
Summarize the following transcript of a podcast in a couple paragraphs at most: {text}
```

# Takeaways

My main learnings through this effort are:

- Whisper is a fantastic transcription tool and running this locally on a Mac using whisper.cpp is quite feasible because the quantized models are small
- I tried various open source LLMs for summarization and they all had trouble generating decent summaries consistently. Some of the issues were
  - summaries are empty strings
  - summaries are generated after repeating the text
  - hallucinations
- Some of the models I tried:
  ...
  - `inkbot-13b-8k-0.2.Q8_0.gguf`
- gpt-4 is (not surprisingly at this point) able to generate decent summaries consistently

# Code

To generate summaries using a local LLM, you can use the following code:
...
def load_llm_llamacpp(
    ...
    **kwargs,
    ...
    """
    Loads a LlamaCpp model from the model directory with
    default parameters for an Apple Silicon Mac
    """

    model_path = str(model_dir / model_name)

    llm_llamacpp = LlamaCpp(
        # model_path has to take a str not a pathlib.Path
        ...
        n_gpu_layers=n_gpu_layers,  # Metal set to 1 is enough.
        # n_batch should be between 1 and n_ctx
        n_batch=n_batch,
...
llm_solar_10b = load_llm_llamacpp(
    model_dir = Path("/Users/lawrencewu/github/research-llms/models/"),
    model_name = "solar-10.7b-instruct-v1.0.Q8_0.gguf"
)

for section in data:
    print(f"Section: {section['title']}")
    prompt = f"Summarize the following transcript of a podcast in a couple paragraphs at most: {section['text']}"
    section["summary_solar_10b"] = llm_solar_10b(prompt)
    print(f"Summary: {section['summary_solar_10b']}")
```

# LLM Summaries

The summaries below are generated by `solar-10.7b-instruct-v1.0.Q8_0.gguf` and gpt-4. Some of the summaries are quite good like the one for Word2Vec and Emergence Mirage. However, some issues with most of the summaries:

- the LLM looks like it is repeating text verbatim from the source text
- in some cases a summary is not generated at all
- sometimes the summary is generated after repeating the text
- there are hallucinations too

For reference I also generated summaries using gpt-4 which is a strong comparison.

## Test of Time Paper: Word2Vec
...
Summary:  this is it.
NeurIPS 2022: A Recap of the Conference in Podcast Form (Part 3)
In this third part of our NeurIPS 2022 recap podcast series, we cover a variety of topics including:

- The challenges and limitations of LLMs in planning tasks, as discussed by Dr. Nada Lavrac from UC San Diego.
- A prefrontal cortex-inspired modular architecture for solving complex problems using GPT-4.
- State space models and their significance, as presented by Chris Ray from Stanford University.
...
an algorithmic music.
a little bit, so we're talking about the music, but it's a lot of music, and that's an algorithmic music, and it's a lot of music. I'm talking about music.
that's a lot of music, and it's a lot of music, and it's a lot of music, so it's a little bit of music, but it's a lot of music, and it's a lot of music.
a lot of music, and this is a lot of music, and it's a lot of music.
it's a lot of music.
music.
a lot of music, and it's a lot of music, and it's a lot of music, and it's a lot of music, and it's a lot of music. It's a lot of music, and it's a lot of music, and it's a lot of music, and it's a lot of music, and it's a lot of music, and it's a lot of music, and it's a lot of music, and it's a lot, and it's a lot, and it's a lot, so it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot of music. It's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot. It's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot. It's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot. It's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot. It's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot. It's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot. It's a little bit, and it's a lot. This is an interesting things like this is, I thinking, it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot. It's a lot, and this is a lot, and it's a lot, and it's interesting things like a lot. But it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and it's a lot, and this is a lot, and it's a lot, and it's a lot, and it's interestingly, and it's interesting things. It's interesting things, but we can see the thing. It's like it's interestingly, and it's interesting things, and it's interesting things. It's interesting things, and this is, and it's interesting, and it's interesting things. It's interesting things. It's interesting things, and it's interesting things, and it's interesting things. It's interesting things, but it's interesting things. It's interesting things. There's interesting things. It's interesting things. So, like this. There'sssss interesting things. I thinks. It's interesting things. This is, and it'sssss interesting things. And it'ssssss - it'ssss because it'ssss very small. So, like a lot of times. It'sssss very, and this. It'ssssss and it'sssss and it'sss interesting things. I thinks, but it'sss and it'sss - because there are interesting things. There'sss and it'ssss - it'ss very, like this, it'ss very, it'ssss and it'sss and it'sss and it'ss interesting things, but it'sss and it'sss interesting things. It'ss and it'ss and it'sss and it'sss interesting things. There'sss and it'ss and it'sss and it'sss and this is, and it'ss and it'ss very, and it'ss and it'ss and it'ss and it'sss and it's interesting things. It's and 4. This. I thinks and we can - that means that there are interesting stuff. That's interestingly.
s. It'ss and this, and it'ss and this. We can you know. There's a lot ofinteresting things. So, like this is and it'sss and I thinks and it'sss and it'sss - it'sss and it'sss and this.
s there'sss interestingly, but we can - it'ss - its interesting. It'ssss interesting. So, it'sss and it'ssss - it'ssss and it'ssss - this. We can do because you know. You can really- this. I thinks and it'sss and it'ssss - that'ss and we's and I guess, like this, and 1.
