...
But let me share the results of my first experiment with Code Interpreter and it's ability to do data analysis.
...
![Relationship between different performance metrics](./nba-use-case-5.png)
...
I found another dataset that had every player's draft year, round and number. I uploaded it to Code Interpreter. The LLM was smart enough to name the dataframe as `df_new` so it didn't overwrite `df` from the previous analysis.
...
I had another idea to look at embeddings of players to see if it could generate similar players. So I asked Code Interpreter to generate embeddings for each player based on basketball statistics and physical attributes (weight, height).
...
- Level 2: AI agent that helps you write code, can reason and debug itself, e.g. `gpt-3.5-turbo` + `langchain` [from a previous post](https://lawrencewu.net/posts/2023-07-12-langchain-agents/)
